...
#!/usr/bin/env python3
"""
GLUE runner for retained-subspace selection in frozen LoRA adapters.

The script trains a LoRA adapter once, scores individual rank directions on a
held-out selection split, evaluates retained top-k subspaces, and optionally runs
small-k exact subset-oracle diagnostics.
"""
...
def _load_glue_raw_with_retry(task: str, cache_dir: Optional[str], logger: logging.Logger, max_retries: int = 3):
    last_exc = None
    for attempt in range(1, max_retries + 1):
        try:
            logger.info("Loading GLUE task=%s (attempt %d/%d)", task, attempt, max_retries)
            return load_dataset("glue", task, cache_dir=cache_dir)
        except Exception as exc:
            last_exc = exc
            logger.warning("GLUE loading failed on attempt %d/%d: %r", attempt, max_retries, exc)
            if attempt < max_retries:
                time.sleep(8 * attempt)
    raise RuntimeError(f"Failed to load GLUE task={task} after {max_retries} attempts: {last_exc!r}")


def _prepare_clean_holdout(raw_train, label_col: str, holdout_ratio: float, split_seed: int):
    return raw_train.train_test_split(test_size=holdout_ratio, seed=split_seed, stratify_by_column=label_col)


def load_glue_dataset(tokenizer, task: str, max_length: int, cache_dir: Optional[str], logger: logging.Logger, split_policy: str = "clean_train_holdout", val_ratio: float = 0.1, test_ratio: float = 0.1, dataset_split_seed: int = 3407):
    raw = _load_glue_raw_with_retry(task, cache_dir=cache_dir, logger=logger)
    single_input_tasks = {"sst2": ["sentence"], "cola": ["sentence"]}
    pair_input_tasks = {
    ...
    if task not in set(single_input_tasks) | set(pair_input_tasks):
        raise ValueError(f"Unsupported GLUE task: {task}")

    if task == "mnli":
        prepared_raw = raw
        val_key = "validation_matched"
        test_key = "validation_mismatched"
    else:
        if split_policy == "clean_train_holdout":
            first = _prepare_clean_holdout(raw["train"], "label", test_ratio, dataset_split_seed)
            train_plus_val = first["train"]
            heldout_test = first["test"]
            relative_val_ratio = val_ratio / max(1e-8, (1.0 - test_ratio))
            second = train_plus_val.train_test_split(test_size=relative_val_ratio, seed=dataset_split_seed + 1, stratify_by_column="label")
            prepared_raw = DatasetDict({
                "train": second["train"],
                "validation": second["test"],
                "test_holdout": heldout_test,
            })
            if "validation" in raw:
                prepared_raw["validation_official"] = raw["validation"]
            val_key = "validation"
            test_key = "test_holdout"
    ...
    if task in single_input_tasks:
        fields = single_input_tasks[task]
        def tokenize_fn(batch):
            return tokenizer(batch[fields[0]], truncation=True, max_length=max_length)
        remove_cols = [c for c in fields + ["idx"] if c in prepared_raw["train"].column_names]
    else:
        fields = pair_input_tasks[task]
        def tokenize_fn(batch):
            return tokenizer(batch[fields[0]], batch[fields[1]], truncation=True, max_length=max_length)
        remove_cols = [c for c in fields + ["idx"] if c in prepared_raw["train"].column_names]

    logger.info("Tokenizing GLUE task=%s ...", task)
    tokenized = prepared_raw.map(tokenize_fn, batched=True, remove_columns=remove_cols)
    tokenized = tokenized.rename_column("label", "labels")
    tokenized.set_format(type="torch")
    tokenized._val_key = val_key
    tokenized._test_key = test_key
    tokenized._task_name = task
    return tokenized


def maybe_subsample(split_ds, n: Optional[int], seed: int):
    if n is None or n <= 0 or n >= len(split_ds):
        return split_ds
    return split_ds.shuffle(seed=seed).select(range(n))


def build_dataloaders(tokenized, collator, batch_size: int, seed: int, max_train: Optional[int], max_val: Optional[int], max_test: Optional[int]):
    val_key = getattr(tokenized, "_val_key", "validation")
    test_key = getattr(tokenized, "_test_key", val_key)
    train_ds = maybe_subsample(tokenized["train"], max_train, seed)
    val_ds = maybe_subsample(tokenized[val_key], max_val, seed)
    test_ds = maybe_subsample(tokenized[test_key], max_test, seed + 999)
...
def compute_direction_table(model: nn.Module, lora_refs: Sequence[LoRAModuleRef], train_loader: DataLoader, val_loader: DataLoader, test_loader: DataLoader, device: torch.device, rank: int, logger: logging.Logger, lambda_v1: float, lambda_v2: float, alpha_v4: float, beta_v4: float, gamma_v4: float, alpha_v5: float, beta_v5: float, gamma_v5: float, js_temperature: float) -> Tuple[pd.DataFrame, Dict[str, float], Dict[str, np.ndarray]]:
    ...
    rows: List[Dict] = []
    val_logits_cache, val_probs_cache = [], []
    labels_cache = None
    for k in range(rank):
        only_mask = torch.zeros(rank, dtype=torch.float32, device=device)
        only_mask[k] = 1.0
        drop_mask = torch.ones(rank, dtype=torch.float32, device=device)
        drop_mask[k] = 0.0
        ...
        rows.append({
            ...
            "only_val_acc": float(only_val["acc"]),
            "only_test_acc": float(only_test["acc"]),
        })
    ...
    df["v5"] = gate * a
    meta = {
        "full_train_acc": full_train["acc"],
        "full_val_acc": full_val["acc"],
        "full_test_acc": full_test["acc"],
        "full_train_loss": full_train["loss"],
        "full_val_loss": full_val["loss"],
        "full_test_loss": full_test["loss"],
    }
    cache = {
        "labels": labels_cache,
        "val_logits": np.stack(val_logits_cache, axis=0),
        "val_probs": np.stack(val_probs_cache, axis=0),
    }
...
def save_ranking_plot(summary_methods: pd.DataFrame, output_dir: Path, task_name: str) -> None:
    ...
    ax.set_title(f"{task_name.upper()} ranking fidelity")
    plt.tight_layout()
    plt.savefig(output_dir / "ranking_fidelity.png", dpi=200)
    plt.close(fig)


def save_topk_curve_plot(summary_topk: pd.DataFrame, output_dir: Path, task_name: str, methods: Sequence[str]) -> None:
    ...
    ax.set_title(f"{task_name.upper()} top-k retention curves")
...
def save_delta_plot(win_rate_df: pd.DataFrame, output_dir: Path, task_name: str, methods: Sequence[str]) -> None:
    ...
    ax.set_title(f"{task_name.upper()} low-budget gain over magnitude")
    ax.legend(ncol=min(4, max(1, len(methods))))
    plt.tight_layout()
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--target-modules", type=str, default="q_lin,v_lin")
    ap.add_argument("--glue-task", type=str, default="sst2", choices=["sst2", "mnli", "cola", "qnli", "mrpc", "qqp", "rte"])
    ...
    ap.add_argument("--split-policy", type=str, default="clean_train_holdout", choices=["official", "clean_train_holdout"])
...
def main() -> None:
    ...
    tokenized = load_glue_dataset(tokenizer, args.glue_task, args.max_length, args.cache_dir, logger, split_policy=args.split_policy, val_ratio=args.val_ratio, test_ratio=args.test_ratio, dataset_split_seed=args.dataset_split_seed)
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        set_seed(seed)
        train_loader, val_loader, test_loader, train_ds, val_ds, test_ds = build_dataloaders(tokenized, collator, batch_size=args.batch_size, seed=seed, max_train=args.max_train_samples if args.max_train_samples > 0 else None, max_val=args.max_val_samples if args.max_val_samples > 0 else None, max_test=args.max_test_samples if args.max_test_samples > 0 else None)
        logger.info("Dataset sizes | train=%d val=%d test=%d", len(train_ds), len(val_ds), len(test_ds))
        num_labels = 3 if args.glue_task == "mnli" else 2
        model = AutoModelForSequenceClassification.from_pretrained(args.model_name, num_labels=num_labels, cache_dir=args.cache_dir)
        ...
        seed_resource = {"seed": seed, "train_seconds": train_seconds, "scoring_seconds": scoring_seconds, "oracle_seconds": oracle_seconds, "seed_total_seconds": seed_total_seconds, **gpu_memory_snapshot()}
        seed_resource_rows.append(seed_resource)
    ...
    save_ranking_plot(summary_methods, output_dir, args.glue_task)
    save_topk_curve_plot(summary_topk, output_dir, args.glue_task, plot_methods)
    save_delta_plot(win_rate_df, output_dir, args.glue_task, plot_methods)
